...
impl<SymbolValue: Debug, AppContext> ParserState<SymbolValue, AppContext> {
    ...
    fn yyreduce(&mut self, reduction: u16, ctx: &mut AppContext) {
        let len = self.tables.yylen[reduction as usize] as usize;
        let lhs = self.tables.yylhs[reduction as usize];

        debug!(
            "state {} reducing by rule {}, len={}, lhs={}",
            self.yystate, self.tables.yyrules[reduction as usize], len, lhs
        );
        ...
        let reduce_value = (self.tables.reduce)(&mut self.value_stack, reduction as usize, ctx);
        ...
        if top_state == 0 && lhs == 0 {
            ...
            let yyn_0 = self.tables.yygindex[lhs as usize];
            let yyn_1 = yyn_0 + self.yystate;

            let next_state: u16 =
                if self.tables.yycheck[yyn_1 as usize] == self.yystate {
                    self.tables.yytable[yyn_1 as usize]
                } else {
                    self.tables.yydgoto[lhs as usize]
                };
            debug!(
    ...
    fn do_defreds(&mut self, ctx: &mut AppContext) -> bool {
        ...
        let mut any = false;
        loop {
            let defred = self.tables.yydefred[self.yystate as usize];
            if defred != 0 {
    ...
    fn try_shift(&mut self, token: u16, lval: SymbolValue) -> bool {
        let shift: i16 = self.tables.yysindex[self.yystate as usize];
        if shift != 0 {
            debug!("try_shift: shift={}", shift);
            let yyn = shift as i32 + (token as i32);
            assert!(yyn >= 0);
            assert!(self.tables.yycheck[yyn as usize] == token);
            let next_state = self.tables.yytable[yyn as usize];
            debug!(
    ...
    fn try_reduce(&mut self, ctx: &mut AppContext, token: u16) -> bool {
        let red = self.tables.yyrindex[self.yystate as usize];
        if red != 0 {
            let yyn = red + (token as u16);
            debug!("    yyn={}", yyn);
            assert!(self.tables.yycheck[yyn as usize] == token);
            debug!("    reducing by {}", red);
            let rr = self.tables.yytable[yyn as usize];
