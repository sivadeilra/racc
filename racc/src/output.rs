...
fn output_gen_methods(symbol_value_ty: Type) -> TokenStream {
    quote! {
        ...
        impl ParserState {
            ...
            fn yyreduce(&mut self, reduction: u16, ctx: &mut AppContext) -> Result<(), racc_runtime::Error> {
                ...
                let reduce_value = reduce_actions(self.value_stack.drain(self.value_stack.len() - len..), reduction, ctx)?;
            ...
            fn try_reduce(&mut self, ctx: &mut AppContext, token: u16) -> Result<bool, racc_runtime::Error> {
            ...
            pub fn finish(&mut self, ctx: &mut AppContext) -> Result<#symbol_value_ty, racc_runtime::Error> {
                assert!(!self.state_stack.is_empty());
