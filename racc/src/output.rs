...
fn output_parser_struct(
    ...
    parse_quote! {
        ...
        pub struct Parser #token_generics {
            yystate: u16,
...
pub(crate) fn output_parser(
    ...
    let mut output_mod: syn::ItemMod = parse_quote! {
        mod this_name_is_not_used {
            ...
            impl #token_generics Default for Parser #token_generics {
                fn default() -> Self {
                    Self {
    ...
    let fake_mod: syn::ItemMod = parse_quote! {
        mod name_is_not_used {
            ...
            #[inline(never)]
            #[cold]
            fn error_invalid_token_in_token_stack(_t: Token) -> ! {
                panic!("unexpected token in token stack");
            }
    ...
    let mut parser_impl: syn::ItemImpl = parse_quote! {
        impl #token_generics Parser #token_generics {
...
/// Builds the `yyreduce()` function and all of its action blocks.
fn output_yyreduce(
    gram: &Grammar,
    yygindex_table: &[i16],
    yydgoto_table: &[i16],
        ...
        for (rhs_sym_or_rule, rhs_binding) in rhs_syms.iter().rev().zip(rhs_bindings.iter().rev()) {
            ...
            if let Some(rbind) = rhs_binding {
                ...
                if let Some(rhs_sym_ty) = rhs_sym_ty_opt {
                    ...
                    if gram.is_var(rhs_sym) {
                        ...
                        if gram.nvars == 2 {
                            ...
                            extend_stmt_parse_quote_spanned! {
                                stmts1, rule_span =>
                                let VarValue::#rhs_ident(#rbind_ident) = self.value_stack.pop().unwrap();
                            }
                        } else {
                            extend_stmt_parse_quote_spanned! {
                                stmts1, rule_span =>
                                let #rbind_ident: #rhs_sym_ty = match self.value_stack.pop().unwrap() {
                        ...
                        if false {
                            let log_msg =
                                format!("popped value {}({}) = {{:?}}", rhs_ident, rbind_ident);
                            stmts1.push(parse_quote_spanned! {
                                rule_span =>
                                racc_log!(#log_msg, #rbind_ident);
                            });
                        }
                    } else {
                        ...
                        extend_stmt_parse_quote_spanned! {
                            stmts1,
                            rule_span =>
                            let #rbind_ident: #rhs_sym_ty = match self.token_stack.pop().unwrap() {
                    ...
                    action_func_params.extend(quote!(mut #rbind_ident: #rhs_sym_ty,));
                    action_func_call_args.extend(quote!(#rbind_ident,));
                } else {
...
fn output_parse(
    ...
    let mut lifetime_inserts = TokenStream::new();
    for lifetime in token_generics.lifetimes() {
        let lifetime_ident = &lifetime.lifetime.ident;
        lifetime_inserts.extend(quote!(#lifetime_ident,));
    }
    ...
    syn::ImplItem::Method(parse_quote! {
        ...
        where
            I: IntoIterator<Item = Token #token_generics>
        {
